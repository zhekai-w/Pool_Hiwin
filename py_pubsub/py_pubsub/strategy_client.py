...
class ArmsDealer_2(Node):
    ...
    def strategy_state(self, state:States) -> States:
        if state == States.INIT:
            ...
            pose = Twist()
            if self.obs_flag == 0:
                [pose.linear.x, pose.linear.y, pose.linear.z] = [self.cue_hitpoint[0], self.cue_hitpoint[1], -162.0]
            else:
                [pose.linear.x, pose.linear.y, pose.linear.z] = [self.cue_hitpoint[0], self.cue_hitpoint[1], -149.0]
            ...
            res = self.call_hiwin(req)
            if res.arm_state == RobotCommand.Response.IDLE:
                nest_state = States.CLOSE_SECOND_IO
            else:
                nest_state = None

        elif state == States.CLOSE_SECOND_IO:
            req = self.generate_robot_request(
                cmd_mode = RobotCommand.Request.DIGITAL_OUTPUT,
                digital_output_cmd = RobotCommand.Request.DIGITAL_OFF,
                digital_output_pin = HEAVY_PIN
