...
def check_obstacle(ballx, bally, vectorx, vectory, objx, objy):
    maskheight = tableheight/actualheight*3.9+2*r
    maskwidth = tablewidth/actualwidth*8.7+2*r
    vectorlengh = math.sqrt(abs(vectorx)**2+abs(vectory)**2)
    ...
    #table lines
    table_line_1 = [(0, 0), (tablewidth, 0)]
    table_line_2 = [(tablewidth, 0), (tablewidth, tableheight)]
    table_line_3 = [(0, tableheight),(tablewidth, tableheight)]
    table_line_4 = [(0, 0), (0, tableheight)]
...
def main(cuex, cuey, objectballx, objectbally, n):
    ...
    if Numberofroute == 0:
        ...
        for i in range(0,n):
            if 0 <= objectballx[i] <= (tablewidth/7)*3 and 0 <= objectbally[i] <= tableheight*2/3:
                countN[0] = countN[0] + 1
                who1.append(i)
            if (tablewidth/7)*2 <= objectballx[i] <= (tablewidth/7)*5 and 0 <= objectbally[i] <= tableheight*2/3:
                countN[1] = countN[1] + 1
                who2.append(i)
            if (tablewidth/7)*4 <= objectballx[i] <= tablewidth and 0 <= objectbally[i] <= tableheight*2/3:
                countN[2] = countN[2] + 1
                who3.append(i)
            if (tablewidth/7)*4 <= objectballx[i] <= tablewidth and tableheight*1/3 <= objectbally[i] <= tableheight:
                countN[3] = countN[3] + 1
                who4.append(i)
            if (tablewidth/7)*2 <= objectballx[i] <= (tablewidth/7)*5 and tableheight*1/3 <= objectbally[i] <= tableheight:
                countN[4] = countN[4] + 1
                who5.append(i)
            if 0 <= objectballx[i] <= (tablewidth/7)*3 and tableheight*1/3 <= objectbally[i] <= tableheight:
                countN[5] = countN[5] + 1
                who6.append(i)
        who.append(who1)
        who.append(who2)
        ...
        print("shortest distance to cue in this area:",min(thisareadis))
...
if __name__ == '__main__':
    ...
    illogical = 1
    while illogical:
        cuex, cuey, objectballx, objectbally, n = generateballs(1, r)
    ...
    objectbally.append(cuey)
    '''
    main(cuex, cuey, objectballx, objectbally, n)
