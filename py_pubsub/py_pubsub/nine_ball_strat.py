...
def check_obstacle(first_poly, second_poly, third_poly, fourth_poly):   
    # polygon
    points_inside = []
    obstacle_flag = 0
...
def check_ball_in_way(first_poly, second_poly, third_poly, fourth_poly):
...
class ReflectPointsVectors:
    def __init__(self, target_hitpoint_x, target_hitpoint_y):
    ...
    def calculate_reflect_points_vectors(self):
        ...
        for i in range(6):
            ...
            target_to_top_dis = abs(aimpointy[0] - self.target_hitpoint_y[i])
            cue_to_top_dis = abs(aimpointy[0] - cuey)
            cue_to_hitpoint_x_dis = self.target_hitpoint_x[i] - cuex
            reflect_point_x = cuex + cue_to_hitpoint_x_dis * cue_to_top_dis / (target_to_top_dis + cue_to_top_dis)
            self.top_reflect_point_x.append(reflect_point_x)
            # calculate top reflection vectors
            self.reflect_top_vectors.append([reflect_point_x - cuex, aimpointy[0] - cuey,
                                             self.target_hitpoint_x[i] - reflect_point_x,
                                             self.target_hitpoint_y[i] - aimpointy[0]])

            # calculate bottom reflection points
            target_to_bot_dis = abs(aimpointy[1] - self.target_hitpoint_y[i])
            cue_to_bot_dis = abs(aimpointy[1] - cuey)
            cue_to_hitpoint_x_dis = self.target_hitpoint_x[i] - cuex
            reflect_point_x = cuex + cue_to_hitpoint_x_dis * cue_to_bot_dis / (target_to_bot_dis + cue_to_bot_dis)
            self.bot_reflect_point_x.append(reflect_point_x)
            # calculate bot reflection vectors
            self.reflect_bot_vectors.append([reflect_point_x - cuex, aimpointy[1] - cuey,
                                             self.target_hitpoint_x[i] - reflect_point_x,
                                             self.target_hitpoint_y[i] - aimpointy[1]])

            # calculate left reflection points
            target_to_left_dis = abs(aimpointx[0] - self.target_hitpoint_x[i])
            cue_to_left_dis = abs(aimpointx[0] - cuex)
            cue_to_hitpoint_y_dis = self.target_hitpoint_y[i] - cuey
            reflect_point_y = cuey + cue_to_hitpoint_y_dis * cue_to_left_dis / (target_to_left_dis + cue_to_left_dis)
            self.left_reflect_point_y.append(reflect_point_y)
            # calculate left reflection vectors
            self.reflect_left_vectors.append([aimpointx[0] - cuex, reflect_point_y - cuey,
                                              self.target_hitpoint_x[i] - aimpointx[0],
                                              self.target_hitpoint_y[i] - reflect_point_y])

            # calculate right reflection points
            target_to_right_dis = abs(aimpointx[3] - self.target_hitpoint_x[i])
            cue_to_right_dis = abs(aimpointx[3] - cuex)
            cue_to_hitpoint_y_dis = self.target_hitpoint_y[i] - cuey
            reflect_point_y = cuey + cue_to_hitpoint_y_dis * cue_to_right_dis / (target_to_right_dis + cue_to_right_dis)
            self.right_reflect_point_y.append(reflect_point_y)
            # calculate right reflection vectors
            self.reflect_right_vectors.append([aimpointx[3] - cuex, reflect_point_y - cuey,
                                               self.target_hitpoint_x[i] - aimpointx[3],
                                               self.target_hitpoint_y[i] - reflect_point_y])
...
def DISPLAY_OBSTACLE_BALL(who_in_cue_to_target_way, who_in_target_to_aim_way):
...
def main(objectballx:list, objectbally:list, cuex:float, cuey:float):
    ...
    for i in range(6):
        ...
        if angles[i] < 80 and outofbound(target_hitpoint_x[i], target_hitpoint_y[i]) == 0:
            ...
            in_cue_target_obs_number, in_cue_target_poly_indices = check_ball_in_way(poly1, poly2, poly3, poly4)
            ...
            if in_cue_target_obs_number == 0:
                ...
                DISPLAY_MASK(poly1, poly2, poly3, poly4)
                in_target_aim_obs_number, in_target_aim_poly_indices = check_ball_in_way(poly1, poly2, poly3, poly4)
    ...
    if len(All_simple_routes) > 0:
        ...
        obstacle_flag, points_in_poly = check_obstacle(first_poly, second_poly, third_poly, fourth_poly)
        print("obstacle flag:", obstacle_flag)
        DISPLAY_MASK(first_poly, second_poly, third_poly, fourth_poly)
        DISPLAY_OBSTACLE_BALL(who_in_cue_to_target_way, who_in_target_to_aim_way)
        DISPLAY_POOL_TABLE()
    ...
    reflect_points_vectors = ReflectPointsVectors(target_hitpoint_x, target_hitpoint_y)
    ...
    for i in range(6):
        ...
        if outofbound(target_hitpoint_x[i], target_hitpoint_y[i]) == 0:
            ...
            if cue_top_and_cue_target_angle < 135 and top_target_and_target_aim_angle < 80:
                ...
                number_of_obstacle_T, in_poly_indices = check_ball_in_way(first_poly_0_T, second_poly_0_T, third_poly_0_T, fourth_poly_0_T)
                if number_of_obstacle_T == 0:
                        ...
                        number_of_obstacle_T, in_poly_indices = check_ball_in_way(first_poly_1_T, second_poly_1_T, third_poly_1_T, fourth_poly_1_T)
                        if number_of_obstacle_T == 0:
                            ...
                            number_of_obstacle_T, in_poly_indices = check_ball_in_way(first_poly_2_T, second_poly_2_T, third_poly_2_T, fourth_poly_2_T)
            ...
            if cue_bot_and_cue_target_angle < 135 and bot_target_and_target_aim_angle < 80:
                ...
                number_of_obstacle_B, in_poly_indices = check_ball_in_way(first_poly_0_B, second_poly_0_B, third_poly_0_B, fourth_poly_0_B)
                if number_of_obstacle_B == 0:
                        ...
                        number_of_obstacle_B, in_poly_indices = check_ball_in_way(first_poly_1_B, second_poly_1_B, third_poly_1_B, fourth_poly_1_B)
                        if number_of_obstacle_B == 0:
                            ...
                            number_of_obstacle_B, in_poly_indices = check_ball_in_way(first_poly_2_B, second_poly_2_B, third_poly_2_B, fourth_poly_2_B)
            ...
            if cue_left_and_cue_target_angle < 135 and left_target_and_target_aim_angle < 80:
                ...
                number_of_obstacle_L, in_poly_indices = check_ball_in_way(first_poly_0_L, second_poly_0_L, third_poly_0_L, fourth_poly_0_L)
                if number_of_obstacle_L == 0:
                        ...
                        number_of_obstacle_L, in_poly_indices = check_ball_in_way(first_poly_1_L, second_poly_1_L, third_poly_1_L, fourth_poly_1_L)
                        if number_of_obstacle_L == 0:
                            ...
                            number_of_obstacle_L, in_poly_indices = check_ball_in_way(first_poly_2_L, second_poly_2_L, third_poly_2_L, fourth_poly_2_L)
            ...
            if  cue_right_and_cue_target_angle< 135 and right_target_and_target_aim_angle < 80:
                ...
                number_of_obstacle_R, in_poly_indices = check_ball_in_way(first_poly_0_R, second_poly_0_R, third_poly_0_R, fourth_poly_0_R)
                if number_of_obstacle_R == 0:
                        ...
                        number_of_obstacle_R, in_poly_indices = check_ball_in_way(first_poly_1_R, second_poly_1_R, third_poly_1_R, fourth_poly_1_R)
                        if number_of_obstacle_R == 0:
                            ...
                            number_of_obstacle_R, in_poly_indices = check_ball_in_way(first_poly_2_R, second_poly_2_R, third_poly_2_R, fourth_poly_2_R)
    ...
    if len(All_reflected_routes) > 0:
        ...
        return [score, cuetoi_v[0], cuetoi_v[1], obstacle_flag, hitpoint[0], hitpoint[1]]
    else:
