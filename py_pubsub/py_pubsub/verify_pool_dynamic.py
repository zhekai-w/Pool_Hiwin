...
ALL_cfg_path = CWD + "/src/py_pubsub/py_pubsub/cfg/yolov4-obj.cfg"      #'./cfg/yolov4-obj.cfg'
ALL_weights_path = CWD + "/src/py_pubsub/py_pubsub/cfg/weights/ALL/yolov4-obj_best.weights"
ALL_data_path = CWD + "/src/py_pubsub/py_pubsub/cfg/hiwin_C_WDA_v4.data"
...
def draw_boxes(detections, image, colors):
    ...
    ball_imformation = [[-999 for _ in range(4)] for _ in range(len(detections))]
    i = 0
...
def dynamic_YOLO_info(first_or_second):
    ...
    # for camera to auto focus, can try to lock camera exposure/ISO
    time.sleep(1)
    confidence = []
    objectballx = []
    objectbally = []
    ...
    if first_or_second == 1:
        ...
        undistorted_image = cv2.undistort(color_img, cameraMatrix, distCoeffs, None)
        detected_img, ballinfo = detect_ALL(undistorted_image)
    ...
    def verification_state(self, state: States) -> States:
        if state == States.INIT:
            ...
            if res.arm_state == RobotCommand.Response.IDLE:
                nest_state = States.FIRST_CALCULATE

        elif state == States.FIRST_CALCULATE:
            ...
            if self.ball_cnt < len(self.objectballx):
                ...
                while True:
                    ...
                    color_img = np.asanyarray(color_frame.get_data())
                    detected_img, all_ball_infos = detect_ALL(color_img)
                    print("Number of balls:",len(all_ball_infos))
                    print("all balls info:", all_ball_infos)

                    '''
                    If all ball info is empty or dev is too big, means the mid ball is not detected
                    error prone.
                    So far use time.sleep for camera to auto adjust.
                    Possible solution:
                        fix camera exposure or wait for mid ball to be detected 
                    '''
                    ...
                    for ball_info in all_ball_infos:
                        dev_x = ball_info[2] - 1920/2
                        dev_y = ball_info[3] - 1080/2
                        temp_error = math.sqrt((dev_x)**2+(dev_y)**2)
                        mid_error.append(temp_error)
                        print("mid error:", mid_error)
                        min_error_index = mid_error.index(min(mid_error))
                        print("min index:", min_error_index)
                        mid_x = all_ball_infos[min_error_index][2]
                        mid_y = all_ball_infos[min_error_index][3]

                    
                    ball_relative_cam = pixel_mm_convert(self.fix_z - abs(tool_to_cam[2]) + abs(self.table_z), [mid_x, mid_y])
                    print("mid:", [mid_x, mid_y])
                    print("dev:", [dev_x, dev_y])
                    print("ball to cam:", ball_relative_cam[0:2])
                    ...
                    if abs(dev_x) <= 3 and abs(dev_y) <= 3:
                        # cv2.destroyAllWindows()
                        break
                # Stop streaming
                pipeline.stop()
                ...
                ball_relative_cam = pixel_mm_convert(self.fix_z - abs(tool_to_cam[2]) + abs(self.table_z), [mid_x, mid_y])
                print("final mid:", [mid_x, mid_y])
                print("final dev:", [dev_x, dev_y])
                req = self.generate_robot_request(cmd_mode=RobotCommand.Request.CHECK_POSE)
                res = self.call_hiwin(req)
                self.second_photo = res.current_position
